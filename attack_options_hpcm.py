...
def get_attack_args():
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "'hpcm' = sample (state,hdr) by difficulty table (default). "
            "'sequential' = ablation: cycle cameras in the existing train_cameras order and also cycle hdr_id round-robin."
        ),
    )
    parser.add_argument(
        "--hpcm_temperature",
    ...
    parser.add_argument(
        ...
        default=True,
        action=argparse.BooleanOptionalAction,
        help="Precompute SH coefficients for each HDR base once at startup.",
    )
    parser.add_argument(
        "--hpcm_enable_relight_cache",
        default=True,
        action=argparse.BooleanOptionalAction,
        help="Cache LBM relighting results for (camera, hdr) pairs to skip redundant computation.",
    )
    parser.add_argument(
        "--hpcm_group_by_hdr",
        default=True,
        action=argparse.BooleanOptionalAction,
        help="Group sampled configs by hdr_id inside each step to enable detector batching.",
    )
    ...
    parser.add_argument(
        ...
        default=False,
        action=argparse.BooleanOptionalAction,
        help="Enable lightweight timing logs (writes CSV under save_dir and prints periodic summaries).",
    )
    parser.add_argument(
        "--profile_interval",
        type=int,
        default=50,
        help="Log timing every N steps/batches when --profile is enabled.",
    )
    ...
    parser.add_argument(
        "--lbm_disk_cache_dir",
        type=str,
        default="./lbm_disk_cache3",
        help="If set, cache LBM relit backgrounds on disk keyed by (camera_name, hdr_id). Useful to reuse across runs.",
    )
    parser.add_argument(
        "--lbm_disk_cache_precompute",
        default=False,
        action=argparse.BooleanOptionalAction,
        help="If enabled and --lbm_disk_cache_dir is set, precompute missing LBM relit backgrounds at startup (can be slow).",
    )
    parser.add_argument(
        "--lbm_disk_cache_force_rebuild",
        default=False,
        action=argparse.BooleanOptionalAction,
        help="If enabled, delete existing disk cache under --lbm_disk_cache_dir and rebuild it during precompute.",
    )
