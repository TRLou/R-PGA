...
def main():
	...
	# Optional: precompute SH coeffs per HDR base (huge speedup vs per-sample computation)
	hdr_sh_cpu: list[torch.Tensor] | None = None
	if bool(getattr(args, "enable_lbm_relight", False)) and bool(getattr(args, "hpcm_precompute_hdr_sh", True)):
		print(f"[消息] [HPCM-Speed] 预计算 HDR->SH（共 {len(hdr_bases_cpu)} 个 EnvMap）...")
		hdr_sh_cpu = []
		for i, base_cpu in enumerate(hdr_bases_cpu):
			try:
				...
				print(f"[警告] [HPCM-Speed] HDR->SH 预计算失败: {hdr_names[i] if i < len(hdr_names) else i}: {e}")
	...
	for step in pbar:
		...
		# ------------------------------------------------------------
		# HPCM-Speed: sample a batch of configs, then group by hdr_id
		# so we can batch detector inference and reduce build_mips/SH overhead.
		# ------------------------------------------------------------
		sampled: list[tuple[int, int, object]] = []  # (config_id, hdr_id, cam)
		for _bi in range(batch_size):
			...
			# choose one camera for this state (your dataset typically has exactly 1 per config)
			cam_indices = cams_by_state[state_id]
			cam_idx = int(rng.choice(cam_indices))
			cam = train_cameras[cam_idx]
			sampled.append((int(config_id), int(hdr_id), cam))

		if not sampled:
			continue

		if bool(getattr(args, "hpcm_group_by_hdr", True)):
			# group by hdr_id
			groups: dict[int, list[tuple[int, object]]] = {}
			for cid, hid, cam in sampled:
				groups.setdefault(int(hid), []).append((int(cid), cam))

			for hid, items in groups.items():
				config_ids_g = [cid for (cid, _cam) in items]
				cams_g = [_cam for (_cid, _cam) in items]
				if not cams_g:
					continue

				# switch envlight base ONCE for this group
				with torch.no_grad():
					base_cpu = hdr_bases_cpu[int(hid)]
					...
					try:
						...
						pass
					# SH conditioning for LBM relighter
					if relighter is not None:
						try:
							if hdr_sh_cpu is not None and 0 <= int(hid) < len(hdr_sh_cpu):
								gaussians.hdr_sh_coeffs = hdr_sh_cpu[int(hid)].to(device)
							else:
				...
				# forward for this group of cameras (detector is batched here)
				group_total_loss, cls_loss, reg_loss, _, vis_data, per_sample_losses = compute_batch_loss(
					cams_g,
					gaussians,
					pipe,
					bg,
				...
				if group_total_loss is None:
					continue

				# Update per-config scores using per-sample losses (aligned with cams_g order)
				if per_sample_losses is None:
					# Fallback: treat group loss as equally shared
					per_sample_losses = group_total_loss.detach() * 0.0 + (group_total_loss / float(max(1, len(cams_g))))
					per_sample_losses = per_sample_losses.repeat(len(cams_g))

				n_eff = int(per_sample_losses.shape[0])
				# Logging: approximate per-sample cls/reg by dividing group sums
				cls_per = float(cls_loss.item()) / float(max(1, n_eff)) if cls_loss is not None else 0.0
				reg_per = float(reg_loss.item()) / float(max(1, n_eff)) if reg_loss is not None else 0.0

				for j in range(min(n_eff, len(config_ids_g))):
					loss_j = per_sample_losses[j]
					lv = float(loss_j.item())
					hpcm.update(config_ids_g[j], lv)
					losses_t.append(loss_j)
					loss_vals.append(lv)
					cls_vals.append(cls_per)
					reg_vals.append(reg_per)

				if first_vis_data is None and vis_data:
					first_vis_data = vis_data[0]
					try:
						first_vis_name = cams_g[0].image_name
					except Exception:
						first_vis_name = None
		else:
			# Original behavior: per-sample forward (slower, but sometimes helpful for debugging)
			for config_id, hdr_id, cam in sampled:
				with torch.no_grad():
					...
					if relighter is not None:
						try:
							if hdr_sh_cpu is not None and 0 <= int(hdr_id) < len(hdr_sh_cpu):
								gaussians.hdr_sh_coeffs = hdr_sh_cpu[int(hdr_id)].to(device)
							else:
								gaussians.hdr_sh_coeffs = base_cubemap_to_sh(gaussians.envlight.base, device)
						except Exception:
							gaussians.hdr_sh_coeffs = torch.zeros(27, dtype=torch.float32, device=device)
