...
def main():
	...
	# 记录 envlight 初始状态（来自 checkpoint 或默认初始化），用于每个 epoch 重置
	envlight_init_state = deepcopy(gaussians.envlight.state_dict())
	...
	for epoch in range(1, args.epochs + 1):
		...
		# 每个 epoch 开始：可选重置 envlight 到初始状态
		if getattr(args, 'reset_envlight_each_epoch', True):
			try:
				gaussians.envlight.load_state_dict(envlight_init_state)
				gaussians.envlight.build_base()
				gaussians.envlight.build_mips()
				print("[消息] [Epoch] 已将 envlight 重置为初始状态并重建 mips。")
			except Exception as e:
				print(f"[警告] [Epoch] 重置 envlight 失败: {e}")
			...
			if phase == 'min':
				...
				if optimizer_max is not None:
					...
					with torch.no_grad():
						...
						ib_prev = gaussians.envlight.init_base.data.clone() if hasattr(gaussians.envlight, 'init_base') else None
