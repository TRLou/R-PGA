# -*- coding: utf-8 -*-
"""
本脚本用于评估在多天气（Multi-Weather）条件下，应用了对抗性纹理的3D高斯模型的 loss landscape，
按 (pitch, distance) 和 (angle, weather) 的组合生成 landscape 图。

功能概述:
- 批量评估：支持同时评估多个根目录下的图像，每个根目录包含不同的天气子文件夹（如'Dusk', 'Night', 'Rain'等）。
- 多检测器支持：可针对一系列预定义的目标检测器（YOLOX, DETR, Faster R-CNN等）进行评估。
- Loss 计算：从已保存的图像计算包含 0.1 权重 reg_loss 的总 loss。
- Landscape 生成：为每个方法生成一张 landscape 图，横坐标为 (pitch, distance)，纵坐标为 (angle, weather)。
"""
...
from utils.main_utils import coco_classes, load_labelme_annotation, compute_iou, calculate_ap_for_target_class
...
REG_LOSS_WEIGHT = 0.0  # reg_loss 权重（设为0，只使用cls_loss）
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='评估 *mw2 数据，生成 loss landscape 图')
    ...
    parser.add_argument('--all_detectors', action=argparse.BooleanOptionalAction, default=True, help='是否评估所有检测器（默认关闭）。若开启则评估所有检测器')
    ...
    parser.add_argument('--output_file', default='evaluation_results_landscape.txt', help='输出 txt 文件名/路径')
    parser.add_argument('--landscape_dir', default='./landscape_out', help='Landscape 图输出目录')
...
def _write_txt_report(
    ...
    """
    lines: list[str] = []
    lines.append("========== Landscape 评估汇总（按 angle, pitch 组合）==========")
...
def main():
    ...
    # 构建数据文件路径（基于exp_eval_dir和检测器）
    data_file = data_dir / f"aggregated_data_{exp_eval_dir.name}_{'_'.join(detectors_to_evaluate)}.json"
    ...
    for method in methods:
        print(f"[INFO] 为方法 {method} 生成 landscape 图...")
        ...
        if X is None or Y is None or Z is None:
            print(f"[WARNING] 方法 {method} 无法生成 3D 数据，跳过")
            continue
    ...
    print(f"[DONE] 所有 landscape 图已保存到：{landscape_dir}")
