...
def main():
	...
	# --- Initialize Global Momentum for Pareto Projection ---
	global_momentum = None
	if args.use_pareto_projection:
		# Initialize with zeros matching the shape of the optimized parameter
		# gaussians._albedo_init is the parameter being optimized
		global_momentum = torch.zeros_like(gaussians._albedo_init, device=device)
		print("[消息] [Pareto] 已初始化全局动量用于梯度投影。")
	...
	for epoch in range(1, args.epochs + 1):
		...
		for batch_idx, cam_batch in pbar:
			...
			# --- Momentum-guided Gradient Projection (Pareto Optimization) ---
			# Gradient Projection for Conflict Mitigation based on Pareto Optimization
			if args.use_pareto_projection and gaussians._albedo_init.grad is not None:
				# Step A: Get Raw Gradients
				g_t = gaussians._albedo_init.grad
				
				# Step B: Flatten & Conflict Detection
				# Flatten both to 1D vectors for dot product
				g_t_flat = g_t.view(-1)
				m_prev_flat = global_momentum.view(-1)
				
				dot_prod = torch.dot(g_t_flat, m_prev_flat)
				
				# Step C: Gradient Projection (Pareto Step)
				g_projected = g_t
				if dot_prod < 0:
					# Conflict detected: current gradient opposes historical consensus.
					# Project onto the orthogonal plane of the momentum.
					m_prev_norm_squared = torch.dot(m_prev_flat, m_prev_flat)
					epsilon = 1e-8
					
					# Formula: g_proj = g_t - (dot / (norm^2 + eps)) * m_prev
					scale = dot_prod / (m_prev_norm_squared + epsilon)
					g_projected = g_t - scale * global_momentum
					
					# Assign projected gradient back to the parameter
					gaussians._albedo_init.grad = g_projected
				
				# Step D: Update Momentum (EMA)
				# m_new = beta * m_prev + (1 - beta) * g_projected
				beta = args.pareto_beta
				global_momentum = beta * global_momentum + (1 - beta) * g_projected.detach()
